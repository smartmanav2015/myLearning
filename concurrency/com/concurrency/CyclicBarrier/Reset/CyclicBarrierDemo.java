...
public class CyclicBarrierDemo {
	public static void main(String args[]) {
		// No.of threads to wait. In general terms, no.of queues to wait here 2
		CyclicBarrier cb = new CyclicBarrier(2, new GoVIP());

		// Create two threads,a 50/- line and a 100/- line
		new FiftyClass(cb);
		new HundredClass(cb);
	}
}
...
class FiftyClass extends Thread {
	...
	public FiftyClass(CyclicBarrier cb) {
		// Know that FiftyCLass has to wait, so is the cyclic barrier. This object is used to call the await()
		this.cb = cb;
		// Start the thread In general terms, let the 50/- queue move
		start();
	}
	...
	public void run() {
		...
		// An InterruptedException is raised. In general, sometimes the situation goes uncontrollable due to heavy rush
		try {
			cb.await();
			System.out.println("so we wee waiting ?");
		} catch (Exception e) {
...
class GoVIP implements Runnable {
	public void run() {
		...
		// Don't forget to say thanks to those 50/- and 100/- queue people!! :)
